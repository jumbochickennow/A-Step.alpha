...
describe('contact delivery integrity', () => {
  it('keeps provider failures retryable and marks delivery only after provider success', async () => {
    ...
    const services = {
      ...
      async archiveEvent(_env: Env, event: { id: string }) {
        archiveCalls += 1;
        if (archiveFails) throw new Error('sheet unavailable');
        return event.id;
      },
    };
    ...
    expect(await env.DB.prepare(
      `SELECT status, attempts, delivered_at, provider_message_id, archive_status
       FROM outbox_events WHERE id = ?1`,
    ).bind(eventId).first()).toMatchObject({
      status: 'failed', attempts: 1, delivered_at: null, provider_message_id: null, archive_status: 'pending',
    });

    providerFails = false;
    await env.DB.prepare('UPDATE outbox_events SET available_at = 0 WHERE id = ?1').bind(eventId).run();
    await expect(deliverOutboxEvent(deliveryEnv, eventId, services as never)).rejects.toThrow('sheet unavailable');
    expect(await env.DB.prepare(
      `SELECT status, attempts, provider_message_id, archive_status, archive_attempts
       FROM outbox_events WHERE id = ?1`,
    ).bind(eventId).first()).toMatchObject({
      status: 'failed', attempts: 2, provider_message_id: 'provider-message-success',
      archive_status: 'failed', archive_attempts: 1,
    });
    ...
    expect(await env.DB.prepare(
      `SELECT status, attempts, delivered_at, provider_message_id, archive_status, archive_attempts, archive_row_id
       FROM outbox_events WHERE id = ?1`,
    ).bind(eventId).first()).toMatchObject({
      status: 'delivered', attempts: 3, provider_message_id: 'provider-message-success',
      archive_status: 'delivered', archive_attempts: 2, archive_row_id: eventId,
    });
