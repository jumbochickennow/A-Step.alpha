...
assert.deepEqual(config.routes, [{ pattern: 'www.astepimmigration.space/*', zone_name: 'astepimmigration.space' }]);
...
assert.deepEqual(config.vars?.GOOGLE_SHEETS_SPREADSHEET_ID, '12H3V0OC8ImOqFSHHWdra8GHrktC8u5xK6lzxFcUDd6c');
assert.ok(config.secrets?.required?.includes('GOOGLE_SHEETS_CLIENT_EMAIL'));
assert.ok(config.secrets?.required?.includes('GOOGLE_SHEETS_PRIVATE_KEY'));
