...
### Lead storage and contact delivery

Cloudflare D1 is the encrypted source of truth for contacts, guide-download leads, and newsletter subscriptions. They are visible to authenticated operators in the admin dashboard; there is no automatic Google Sheet copy. Only Contact Us submissions trigger email; Cloudflare Email Sending delivers them to `contact@astepimmigration.space`. Failed email delivery stays retryable in D1 and is never marked delivered early.

Operational records are reviewed after 24 months and removed when they are no longer required.
